...
public class PersonProcessing {
    private static void processPersons(
    		List<Person> list, 
            Predicate<Person> isValid, 
            Consumer<Person> consumer) {
        for (Person person : list) { // for every person in list
            if (isValid.test(person)) {   // is person valid?
                consumer.accept(person); // process person
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        // print only persons of legal age
        Predicate<Person> ofLegalAge = p -> p.getAge() >= 18;
        ...
        processPersons(persons, 
            ...
            System.out::println); // same as: p -> System.out.println(p)
    }
}
